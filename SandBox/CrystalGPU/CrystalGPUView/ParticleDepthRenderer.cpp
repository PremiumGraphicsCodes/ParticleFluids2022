...
ParticleDepthRenderer::ParticleDepthRenderer() :
	shader(nullptr)
{
}

ShaderBuildStatus ParticleDepthRenderer::build(GLObjectFactory& factory)
{
	ShaderBuildStatus status;
	status.isOk = true;

	ShaderUnit vs;
	vs.compileFromFile("./GLSL/ParticleDepth.glvs", ShaderUnit::Stage::VERTEX);

	ShaderUnit fs;
	fs.compileFromFile("./GLSL/ParticleDepth.glfs", ShaderUnit::Stage::FRAGMENT);
...
void ParticleDepthRenderer::release(GLObjectFactory& factory)
{
