...
SSReflectionRenderer::SSReflectionRenderer() :
	shader(nullptr)
{
}

ShaderBuildStatus SSReflectionRenderer::build(GLObjectFactory& factory)
{
	ShaderBuildStatus status;
	status.isOk = true;

	ShaderUnit vs;
	vs.compileFromFile("./GLSL/SSReflection.glvs", ShaderUnit::Stage::VERTEX);

	ShaderUnit fs;
	fs.compileFromFile("./GLSL/SSReflection.glfs", ShaderUnit::Stage::FRAGMENT);
...
void SSReflectionRenderer::release(GLObjectFactory& factory)
{
	factory.remove(shader);
}

void SSReflectionRenderer::render()
{
	shader->bind();
